 ...
 *
 * This script:
 * 1. Renames @providerprotocol/agents -> core
 ...
 * - @providerprotocol/agents -> core
 * - index/ directory -> core/ (typedoc names main entry as "index")
 * - README.md -> index.md
 * - Lowercase everything
 */
function transformPath(relativePath: string): string {
  return relativePath
    .replace(/@providerprotocol\/agents/g, "core")
    // Rename "index/" directory to "core/" (but not index.md files)
    .replace(/^index\//, "core/")
...
function extractTitle(content: string, filename: string): string {
  ...
  if (name.toLowerCase() === "index" || name.toLowerCase() === "readme") {
    return "@providerprotocol/agents";
  }
  return name;
}
...
function fixLinks(content: string): string {
  // Fix relative links
  return content
    // Fix @providerprotocol/agents references in links
    .replace(/\]\(([^)]*@providerprotocol\/agents[^)]*)\)/g, (_, link) => {
      const newLink = link.replace(/@providerprotocol\/agents/g, "core");
      return `](${newLink})`;
    })
    // Fix index/ -> core/ in links
    .replace(/\]\(([^)]*)(\.\.\/index\/|\/index\/|^index\/)([^)]*)\)/g, (_, prefix, indexPath, suffix) => {
      const newPath = indexPath.replace(/index\//g, "core/");
